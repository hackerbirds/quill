...
class HTMLCompiler:
    ...
    def parseLine(self, line):
        ...
        if escaped_line == "\n":
            ...
            text = escaped_line.rstrip()[2:]
            return "<div class=\"f-v2\">\n\t<div class=\"bird neutral\"></div>\n\t<p>"+self.parseInline(text)+"</p>\n</div>\n"
            ...
            if " | " in escaped_line:
                ...
                return "<div class=\"attachment-div\">\n\t<img onclick=\"window.open(\'"+url+"\', \'_blank\');\" class=\"attachment\" src=\""+url+"\""+text+"/>"+self.parseInline(img_quote_html)+"</div>\n"
            else: 
                split_line = escaped_line.rstrip().split(" ")
                url = split_line[1]
                text = ""
                ...
                return "<img class=\"attachment\" src=\""+url+"\" alt=\""+self.parseInline(text)+"\"/>\n"
        # Link/URL (" => ")
        elif escaped_line.startswith("=&gt; ") or escaped_line.startswith("=> "):
            split_line = escaped_line.rstrip().split(" ")
            url = split_line[1]
            ...
            return "<a class=\"arrow\" href=\""+url+"\">"+self.parseInline(text)+"</a>\n<br>\n"
        # List element
        elif escaped_line.startswith("* "):
        ...
        # Blockquote (" > ")
            ...
            return "<blockquote>\n\t<p>"+self.parseInline(escaped_line[4:]).rstrip()+"</p>\n</blockquote>\n"
        # Bar line
        elif escaped_line == "---\n":
            return "<hr>\n"
        else:
